...
-- Deletes the on-disk .sqlite3 (+ -wal/-shm/-backup siblings) for any
-- language that currently has zero cards. There's no "was this the last
-- card in this language" check on delete, so an emptied-out language's file
-- just sits there and keeps showing an empty tab in the card list forever
-- -- confirmed still present after several app restarts. Meant to be called
-- once at plugin init (see main.lua), a logical low-risk point since it
-- runs before the card list UI has been shown for this session. Never
-- touches the legacy (nil-language) DB. Restores whatever language was
-- active before it ran.
function DB.pruneEmptyLanguages()
    local languages = DB.listLanguages()
    if #languages == 0 then return end
    local original_language = DB.getActiveLanguage()
    for _, lang in ipairs(languages) do
        DB.setLanguage(lang)
        local count = DB.getCardCountForBook(nil)
        if count == 0 then
            local path = dbPathForLanguage(lang)
            os.remove(path)
            os.remove(path .. "-wal")
            os.remove(path .. "-shm")
            os.remove(backupPathForLanguage(lang))
            initialized_languages[lang] = nil
            logger.info("vocabdeck: pruned empty language DB", lang)
        end
    end
