...
-- Plugin directory — resolved once at module load time. The debug.getinfo
-- lookup is already device-agnostic (introspects this file's own loaded
-- path on any platform) and is expected to succeed in practice; the
-- fallback only matters if that ever returns nil. Was hardcoded to
-- Kindle's path (/mnt/us/extensions/simpleui.koplugin) -- fixed to use
-- KOReader's own plugin-path convention instead, so the fallback (if it's
-- ever actually hit) is correct on every platform, not just Kindle.
local _plugin_dir = (debug.getinfo(1, "S").source or ""):match("^@?(.+)/[^/]+$")
local function _fallback_plugin_dir()
    local ok, DataStorage = pcall(require, "datastorage")
    if ok and DataStorage then
        return DataStorage:getFullDataDir() .. "/plugins/simpleui.koplugin"
    end
    return "/mnt/us/extensions/simpleui.koplugin" -- last-resort, pre-fix behavior
end
_plugin_dir = _plugin_dir or _fallback_plugin_dir()
