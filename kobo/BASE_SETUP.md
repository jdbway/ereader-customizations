...
The mechanism for all of this — how the chip bar and micromodules work,
the `djb2` chip-hash script, the per-chip download-folder gotcha — is
pure `bookshelf.koplugin` behavior, identical on Kindle, so it's
documented once in
**[`../shared/koreader-plugins/bookshelf.koplugin/SETUP.md`](../shared/koreader-plugins/bookshelf.koplugin/SETUP.md)**
rather than duplicated here. Read that first, then come back for Kobo's
specifics below. Do this after the plugin is installed and `Books/`
exists (previous section).

- **Gesture corner used here**: **Bottom Left**. It defaulted to
  **Screen and lights → Toggle frontlight** — had to explicitly uncheck
  that, per the general note in SETUP.md.
- **Start menu contents**: **not decided yet** — placeholder. Current
  on-device state (from the live rebuild, 2026-08-29) has the stock
  seeded set (quote of the day, reading calendar, toggle Wi-Fi, toggle
  night mode, Bookshelf menu, exit bookshelf, close book, sleep) —
  revisit once there's a real preference and document the chosen set
  here.
- **Chip bar**: the "auto-appearing" Calibre chip on the 2026-08-29
  rebuild wasn't automatic — it (and removing Series/Favourites) was
  done by hand, same mechanism as SETUP.md describes. Can also be done
  as a file edit — see `koreader-settings/bookshelf-tabs-template.lua`
  in this folder (merge just its `tabs` key into a device's real
  `settings/bookshelf.lua`, don't overwrite the whole file).
  `luajit` for the chip-hash script lives at
  `/mnt/onboard/.adds/koreader/luajit`.
- **Per-chip download folder**: hit the exact trap SETUP.md warns about,
  twice. First time (2026-08-29, initial rebuild): the global "last
  used" folder turned out to be the Wallabag folder, so Calibre-chip
  downloads landed in `Books/Wallabag/` instead of `Books/` — fixed by
  setting `download_dir = "/mnt/onboard/Books"` on the chip (already in
  the template above). It **regressed the same day**: the chip actually
  in live use had been recreated by hand via the normal chip-bar flow,
  which doesn't set `download_dir` — only the separate Edit step does,
  and that step was never performed. Fixed again directly on-device.
  Confirmed fixed by a real download landing in `Books/` on 2026-08-29.
- **Micromodules**: matches the Kindle — Clock (digital) + Reading
  streak, tap action set to **Reading insight**. Needed
  `readinginsights.koplugin` installed for that tap action to do
  anything (see the plugin list above) — this was the actual root cause
  when the Reading streak tap didn't do anything on 2026-08-29, found
