...
-- Fetches (or returns cached/saved) memory-helper text for `card`, WITHOUT
-- showing any UI of its own beyond AIRunner's brief "Building..." trap
-- message -- unlike showForCard, this never opens its own TextViewer, so a
-- caller can fold the result into a combined view instead of stacking a
-- second popup on top. `on_done(text)` is always called exactly once;
-- `text` is "" if nothing could be produced (no provider, AI error, or
-- cancel).
--
-- Works for a card that isn't saved yet (no `card.id`) too -- generates
-- ephemerally: no cache-by-id lookup, no DB write, just sets
-- `card.ai_memory_helper` on the table passed in so the caller's own
-- render can pick it up. If the card DOES have an id (already saved),
-- behaves as before: DB/cache-backed, persists on success.
function MemoryHelper.generate(plugin, card, on_done)
    if not card then
        if on_done then on_done("") end
        return
    end
    if card.ai_memory_helper and card.ai_memory_helper ~= "" then
        if on_done then on_done(card.ai_memory_helper) end
        return
    end
    if card.id then
        local cache_entry = _memory_cache[card.id]
        if cache_entry and cache_entry.expires_at > os.time() then
            if on_done then on_done(cache_entry.response) end
            return
        end
    end
    ...
    AIRunner.run(plugin, {
        ...
        on_finally = function()
            finish("") -- catches USER_CANCELED and any other path that skipped success/error
        end,
    })
end
