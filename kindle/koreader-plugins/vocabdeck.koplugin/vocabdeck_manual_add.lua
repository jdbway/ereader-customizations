...
-- Same fast check used throughout the plugin (isWifiOn, not isOnline --
-- see vocabdeck_ai_runner.lua for why). Online, enrich immediately with no
-- extra tap; offline, just leave the card pending -- "Fetch AI data for
-- all cards" picks it up later, same as every other offline-added card.
local function afterManualSave(plugin, card_id)
    local NetworkMgr = require("ui/network/manager")
    if NetworkMgr:isWifiOn() then
        fetchAIForCard(plugin, card_id)
    else
        UIManager:show(InfoMessage:new{
            text = _("Card added. No Wi-Fi -- fetch AI data later from the Actions menu."),
            timeout = 3,
        })
    end
end
...
local function saveManualCard(plugin, phrase, source_language, touchmenu_instance, options)
    ...
    local params = {
        ...
        after_save = function(card_id)
            ...
            afterManualSave(plugin, card_id)
        end,
    }
