...
-- (confirmed via timestamped logging while diagnosing the "Add to
-- vocabulary builder" hook hanging with WiFi off -- the gap between
-- starting and isOnline() reporting false was a consistent ~20s). This is
-- the single shared entry point for every AI action in the plugin ("VD
-- +AI", "Refetch AI data", "AI memory helper", bulk enrichment), so fixing
-- it once here covers all of them. isWifiOn() is a plain local state check
-- with no network round-trip; when WiFi's definitively off, this jumps
-- straight to the same "Do you want to turn on Wi-Fi?" prompt
-- runWhenOnline would eventually reach anyway. When WiFi is already on, the
-- normal runWhenOnline still runs (isOnline() resolves quickly against a
-- real connection, and that path needs the actual-Internet check, not just
-- isWifiOn's radio-state check).
